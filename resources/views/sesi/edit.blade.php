...
@section('title', 'Edit Sesi Absen - SiSantri')

@section('header', 'Edit Sesi Absen')

@section('content')
    <div class="bg-white p-6 rounded-lg shadow">
        <form action="{{ route('sesi.update', $sesi) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="nama" class="block text-sm font-medium text-gray-700 mb-2">Nama Sesi</label>
                <input type="text" name="nama" id="nama" value="{{ old('nama', $sesi->nama) }}"
            ...
            <div class="mb-4">
                ...
                <input type="time" name="waktu" id="waktu"
                    value="{{ old('waktu', \Carbon\Carbon::parse($sesi->waktu)->format('H:i')) }}"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                    required>
                @error('waktu')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="mb-4">
                <label class="flex items-center">
                    <input type="checkbox" name="aktif" value="1" {{ old('aktif', $sesi->aktif) ? 'checked' : '' }}
