        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">{{ \Carbon\Carbon::parse($sesi->waktu)->format('H:i') }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="px-2 py-1 {{ $sesi->aktif ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }} rounded-full text-xs">
                            {{ $sesi->aktif ? 'Aktif' : 'Nonaktif' }}
                        </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex space-x-2">
                            <a href="{{ route('sesi.edit', $sesi) }}" class="px-2 py-1 bg-yellow-100 text-yellow-800 rounded-md">Edit</a>

                            <form action="{{ route('sesi.toggle-active', $sesi) }}" method="POST" class="inline">
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="px-2 py-1 {{ $sesi->aktif ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800' }} rounded-md">
                                    {{ $sesi->aktif ? 'Nonaktifkan' : 'Aktifkan' }}
                                </button>
                            </form>

                            <form action="{{ route('sesi.destroy', $sesi) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus sesi ini?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="px-2 py-1 bg-red-100 text-red-800 rounded-md">Hapus</button>
                            </form>
                        </div>
                    </td>
                </tr>
