...
@section('content')
    <div class="mb-4 flex justify-between items-center">
        <div>
            <h3 class="text-lg font-semibold">Absensi: {{ $sesiAbsen->nama }} -
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <select name="status[{{ $santri->id }}]" class="px-2 py-1 border rounded-md w-full">
                                        <option value="hadir"
                                            {{ isset($existingAbsensis[$santri->id]) && $existingAbsensis[$santri->id]->status === 'hadir' ? 'selected' : '' }}>
                                            Hadir</option>
                                        <option value="izin"
                                            {{ isset($existingAbsensis[$santri->id]) && $existingAbsensis[$santri->id]->status === 'izin' ? 'selected' : '' }}>
                                            Izin</option>
                                        <option value="sakit"
                                            {{ isset($existingAbsensis[$santri->id]) && $existingAbsensis[$santri->id]->status === 'sakit' ? 'selected' : '' }}>
                                            Sakit</option>
                                        <option value="alfa"
                                            {{ isset($existingAbsensis[$santri->id]) && $existingAbsensis[$santri->id]->status === 'alfa' ? 'selected' : '' }}>
                                            Alfa</option>
                                        <option value="piket"
                                            {{ isset($existingAbsensis[$santri->id]) && $existingAbsensis[$santri->id]->status === 'piket' ? 'selected' : '' }}>
                                            Piket</option>
                                    </select>
                                </td>
                            </tr>
