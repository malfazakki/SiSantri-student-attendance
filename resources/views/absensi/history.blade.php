                    ...
                    <select name="sesi_absen_id" id="sesi_absen_id"
                        ...
                        @foreach ($sesiAbsens as $sesi)
                            <option value="{{ $sesi->id }}"
                                {{ request('sesi_absen_id') == $sesi->id ? 'selected' : '' }}>{{ $sesi->nama }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex items-end">
                    ...
                    @if (request('sesi_absen_id') || request('tanggal'))
                        <a href="{{ route('absensi.history') }}"
                            class="ml-2 px-4 py-2 bg-gray-200 text-gray-700 rounded-md">Reset</a>
                    @endif
                </div>
            </div>
        </form>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($absensis as $absensi)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $absensi->santri->nama }}
                                ({{ $absensi->santri->nis }})
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 py-1 {{ $absensi->status === 'hadir' ? 'bg-green-100 text-green-800' : ($absensi->status === 'izin' || $absensi->status === 'sakit' || $absensi->status === 'piket' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800') }} rounded-full text-xs">
